...
class TestCourierCreate:
    ...
    def test_create_courier_missing_fields(self, helpers):
        ...
        # Без имени (необязательное поле, должно создаться)
        payload = {
            "login": helpers.generate_random_string(10),
            "password": helpers.generate_random_string(10)
        }
    ...
    @allure.description("Проверка ошибки при попытке использовать занятый логин")
    def test_create_courier_with_existing_login_error(self, helpers):
        courier_data = helpers.register_new_courier()
        assert courier_data is not None
